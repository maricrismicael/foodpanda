      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav mr-auto">
          ...
          <li class="nav-item dropdown">
            ...
            $sql = "SELECT RESTAURANT_NAME, RESTAURANT_ID FROM `vendors`"; 
            $result = mysqli_query($conn, $sql);
            while($row = mysqli_fetch_assoc($result)){
              echo '<a class="dropdown-item" href="viewFoodList.php?catid=' .$row['RESTAURANT_ID']. '">' .$row['RESTAURANT_NAME']. '</a>';
            }
            echo '</div>
          </li>
          <li class="nav-item">
        ...
        $countsql = "SELECT SUM(`ITEM_QUANTITY`) FROM `viewcart` WHERE `FK_CUSTOMER_ID`=$userId"; 
        ...
        if($loggedin){
          echo '<ul class="navbar-nav mr-2">
            <li class="nav-item dropdown">
              <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown"> Welcome ' .$email. '</a>
              <div class="dropdown-menu" aria-labelledby="navbarDropdown">
