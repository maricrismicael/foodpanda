...
<body onload="showRemove()">
    ...
    <?php
        $id = $_GET['catid'];
        $sql = "SELECT * FROM `vendors` INNER JOIN locations ON vendors.FK_LOCATION_ID = locations.LOCATION_ID WHERE RESTAURANT_ID = $id";
        $result = mysqli_query($conn, $sql);
        while($row = mysqli_fetch_assoc($result)){
            $catname = $row['RESTAURANT_NAME'];
            $catdesc = $row['RESTAURANT_DESC'];
            $city = $row['CITY'];
    ...
    <div class="container my-3" id="cont">
        <div class="col-lg-4 text-center bg-light my-3" style="margin:auto;border-top: 2px groove black;border-bottom: 2px groove black;">     
            ...
            <h6 id="restau-desc"></h6>
            
        </div>
        <h6 id="restau-loc" style="text-align: center;"></h6>
        <div class="row">
        <?php
        include 'partials/_remove_Modal.php';
            $id = $_GET['catid'];
            $sql = "SELECT * FROM `goods` WHERE FK_RESTAURANT_ID = $id";
            $result = mysqli_query($conn, $sql);
            $noResult = true;
            while($row = mysqli_fetch_assoc($result)){
                $noResult = false;
                $foodId = $row['ITEM_ID'];
                $foodName = $row['ITEM_NAME'];
                $foodPrice = $row['UNIT_PRICE'];
                ...
                echo '<div class="col-xs-3 col-sm-3 col-md-3">
                        <div class="card" style="width: 18rem;">
                            ...
                            <div class="card-body">
                                ...
                                <h6 style="color: #ff0000">Php '.$foodPrice. '</h6>  
                                <p class="card-text">' . substr($foodDesc, 0, 29). '...</p>  
                                <div class="row justify-content-center">';
                                if($loggedin){
                                    $quaSql = "SELECT `ITEM_QUANTITY` FROM `viewcart` WHERE FK_ITEM_ID = '$foodId' AND `FK_CUSTOMER_ID`='$userId'";
                                    $quaresult = mysqli_query($conn, $quaSql);
                                    $quaExistRows = mysqli_num_rows($quaresult);
                                    if($quaExistRows == 0) {
                                        echo '<form action="partials/_manageCart.php" method="POST">
                                              <input type="hidden" name="itemId" value="'.$foodId. '">
                                              <input type="hidden" name="foodVendor" value="'.$id.'">
    ...
    <button type="button" id="removeButton" class="btn btn-primary" data-toggle="modal" data-target="#remove_Modal" hidden>Launch demo modal</button>
    
    <script>
        function showRemove(){
            var answer = sessionStorage.getItem('removeModal');
            if(answer==1){
    ...
    <script> 
        ...
        document.getElementById("restau-desc").innerHTML = "<?php echo $catdesc; ?>"; 
        document.getElementById("restau-loc").innerHTML = "<?php echo $street;  ?>, <?php echo $city;  ?>"; 
    </script> 
</body>
</html>